...
# 1. Make sure you have a certificate (e.g. adfs.contoso.local.pfx) either self created or signed by VerifSign, Go Daddy, DigiCert, and etc.
...
# https://adfs.contoso.local/adfs/ls/idpinitiatedsignon.htm

###############################################
# Note:
# If you don't have a public signed certificate (e.g.adfs.contoso.local.pfx) by VerifSign, Go Daddy, DigiCert, and etc.
# Here are manual steps to create a self singed test certificate adfs.contoso.local.pfx
...
# 4. Run command prompt as admin to use my fake root certificate authority to generate a certificate for adfs.contoso.local
...
#	 Expand to /Certificates (Local Computer)/Personal/Certificate/adfs.contoso.local 
