...
if ($templateRootUriString -eq $null) {
  $templateRootUriString = "https://raw.githubusercontent.com/devkeydet/template-building-blocks/master/"
}
...
##########################################################################
#  Manual steps to create a fake root certificate and and use it to create adfs.contoso.local.pfx
...
#  5. Run command prompt as admin to use my fake root certificate authority to generate a certificate for adfs.contoso.local
#        makecert -sk pkey -iv MyFakeRootCertificateAuthority.pvk -a sha256 -n "CN=adfs.contoso.local , CN=enterpriseregistration.contoso.local" -ic MyFakeRootCertificateAuthority.cer -sr localmachine -ss my -sky exchange -pe
#  6. Start MMC certificates console, expand to /Certificates (Local Computer)/Personal/Certificate/adfs.contoso.local and export the certificate with the private key to 
#        C:/temp/adfs.contoso.local.pfx
# ###############################################
# Install certificate to the ADFS and ADFS Proxy VMs:
# 1. Make sure you have a certificate adfs.contoso.local.pfx either self created or signed by VerifSign, Go Daddy, DigiCert, and etc.
# 2. RDP to the each ADFS VM adfs1, adfs2, ...and each ADFS Proxy VM proxy1, proxy2, ...
# 3. Copy to c:\temp the following file
#		c:\temp\adfs.contoso.local.pfx 
#       c:\MyFakeRootCertificateAuthority.cer  (if you created the above cert yourself )
# 4. Run the following command prompt as admin:
#    	certutil.exe -privatekey -importPFX my C:\temp\adfs.contoso.local.pfx NoExport
...
#      \Certificates (Local Computer)\Personal\Certificates\adfs.contoso.local
...
if ($Mode -eq "Adfs") {
	...
	Write-Host
	Write-Host  "3. Browse to https://adfs.contoso.local/adfs/ls/idpinitiatedsignon.htm"
}
...
if ($Mode -eq "Proxy1" ) {
	...
	Write-Host
	Write-Host  "      11.22.33.44 adfs.contoso.local (note: assume the ip is 11.22.33.44)"
	Write-Host
	...
	Write-Host
	Write-Host  "     https://adfs.contoso.local/adfs/ls/idpinitiatedsignon.htm"
	Write-Host
...
if ($Mode -eq "Proxy2" ) {
	...
	Write-Host  
	Write-Host  "Please browse to https://adfs.contoso.local/adfs/ls/idpinitiatedsignon.htm from your development machine to test the adfs proxy installation. You may want to stop proxy1"
	Write-Host  
